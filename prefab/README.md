...
Since the `eag_v3` project uses `uv` for dependency management, you should ensure these packages are installed into the local virtual environment:

```bash
# From the root of eag_v3, run:
uv pip install prefab-ui fastmcp google-genai python-dotenv
```

Prefab needs Python 3.10+. For lessons C and D you also need `GEMINI_API_KEY` in a `.env` file inside this `prefab` directory (which has already been copied over from `s3_code`).

## How to work through each lesson

Because these packages are in the local `.venv`, you need to run them using `uv run` to ensure they execute inside the correct environment. 

1. Read the lesson's `README.md` end to end.
2. Open the lesson's code file and read it top to bottom.
3. Run the scripts explicitly using the environment:
   - For regular scripts: `uv run python [script_name.py]`
   - For the prefab server: `uv run prefab serve [script_name.py]`
